...
function doPost(e){
  ...
  try{
    ...
    if(a==='createPosBill')return requireRole_(session,'Support')?createPosBill_(b.payload||{},session):forbidden_();
...
/*
 * PMT PRIORITY PATCH #1 + #2
 * Paste these helpers into Code.gs and add the two router branches shown below.
 * Do NOT create a second doGet/doPost. Add only the marked branches to the existing router.
 *
 * Script Properties required for MSG91:
 * PMT_MSG91_AUTHKEY
 * PMT_MSG91_SMS_FLOW_ID
 * PMT_MSG91_WA_INTEGRATED_NUMBER
 * PMT_MSG91_WA_TEMPLATE_NAME
 * PMT_MSG91_WA_TEMPLATE_LANG       (default: en)
 * PMT_OWNER_PHONE                  (10-digit Indian mobile)
 *
 * SMS template should contain one variable for order id and one for amount.
 * WhatsApp template should contain two body variables: order id and amount.
 */

const PMT_POS_ORDER_EXTRA_HEADERS = [
  'subtotal','gstRate','gstAmount','paymentDetails','idempotencyKey','source'
];

function ensurePosOrderSchema_(){
  const s=S('Orders');
  if(!s) throw Error('Orders sheet missing');
  ensureOrderSchema_(s);
  const last=Math.max(s.getLastColumn(), ORDER_HEADERS.length + PMT_POS_ORDER_EXTRA_HEADERS.length);
  const h=s.getRange(1,1,1,last).getValues()[0];
  let changed=false;
  for(let i=0;i<PMT_POS_ORDER_EXTRA_HEADERS.length;i++){
    const col=ORDER_HEADERS.length+i;
    if(!h[col]){ h[col]=PMT_POS_ORDER_EXTRA_HEADERS[i]; changed=true; }
  }
...
function pmtJson_(o){
  try{return JSON.stringify(o==null?{}:o);}catch(e){return '{}';}
}

function normalizePayment_(payment,total){
  const p=payment&&typeof payment==='object'?payment:{};
  const mode=clean_(p.mode||'Cash',30);
  let cash=Math.max(0,Number(p.cash)||0);
  let upi=Math.max(0,Number(p.upi)||0);
  let card=Math.max(0,Number(p.card)||0);
  if(mode==='Cash') cash=total;
  if(mode==='UPI') upi=total;
  if(mode==='Card') card=total;
  if(mode==='Cash+UPI') card=0;
  const paid=Math.round((cash+upi+card)*100)/100;
  const target=Math.round(Number(total||0)*100)/100;
  if(Math.abs(paid-target)>0.01){
    throw Error('Payment split must equal bill total');
  }
  return {mode,cash,upi,card,paid};
}

/*
 * Atomically validates and deducts stock for a list of website products.
 * LockService makes two simultaneous devices serialize this critical section.
 * Returns a normalized item list using SERVER prices, never client-supplied prices.
 */
function reserveStockAtomic_(items){
  if(!Array.isArray(items)||!items.length)throw Error('At least one item is required');
  const s=S('Products');
  if(!s)throw Error('Products sheet missing');
  ensureProductSchema_(s);
  const rows=s.getDataRange().getValues();
  const byId={};
  for(let i=1;i<rows.length;i++){
    const p=productMap_(rows[i]);
    if(p.id)byId[p.id]={row:i+1,p};
  }

  const requested={};
  items.forEach(raw=>{
    const id=clean_(raw&&raw.id,120);
    const qty=Math.floor(Number(raw&&raw.qty)||0);
    if(!id||qty<1)throw Error('Invalid cart item');
    const key=id+'::'+clean_(raw&&raw.variantId,80);
  ...
  Object.keys(requested).forEach(key=>{
    ...
    const id=parts[0],variantId=parts.slice(1).join('::');
    const found=byId[id];
    if(!found||found.p.status==='Archived'||found.p.deletedAt)throw Error('Product not available: '+id);
    const qty=requested[key];
    let unitPrice=Number(found.p.price||0);
    if(variantId){
      const v=(found.p.variants||[]).find(x=>String(x.id)===String(variantId));
      if(!v)throw Error('Variant not found: '+id);
      const stock=Math.max(0,Number(v.stock)||0);
      if(stock<qty)throw Error('Insufficient stock for '+found.p.name+' ('+stock+' left)');
      unitPrice=Number(v.price||unitPrice);
    }else{
      const stock=Math.max(0,Number(found.p.stock)||0);
      if(stock<qty)throw Error('Insufficient stock for '+found.p.name+' ('+stock+' left)');
    }
    normalized.push({id,name:found.p.name,qty,price:unitPrice,variantId:variantId||''});
  });

  /* Re-read after validation while the lock is held, then deduct. */
  normalized.forEach(it=>{
    const found=byId[it.id];
    if(it.variantId){
      const variants=found.p.variants||[];
      const idx=variants.findIndex(v=>String(v.id)===String(it.variantId));
      if(idx<0)throw Error('Variant disappeared: '+it.id);
      const current=Math.max(0,Number(variants[idx].stock)||0);
      if(current<it.qty)throw Error('Insufficient stock for '+found.p.name);
      variants[idx].stock=current-it.qty;
      s.getRange(found.row,12).setValue(JSON.stringify(variants));
    }else{
      const current=Math.max(0,Number(s.getRange(found.row,5).getValue())||0);
      if(current<it.qty)throw Error('Insufficient stock for '+found.p.name);
      s.getRange(found.row,5).setValue(current-it.qty);
    }
    s.getRange(found.row,7).setValue(now_());
  });
  return normalized;
}

function restoreStockAtomic_(items){
  if(!Array.isArray(items)||!items.length)return;
  const s=S('Products');if(!s)return;
  ensureProductSchema_(s);
  const rows=s.getDataRange().getValues();
  const byId={};
  for(let i=1;i<rows.length;i++){const p=productMap_(rows[i]);byId[p.id]={row:i+1,p};}
  items.forEach(it=>{
    const f=byId[String(it.id||'')];if(!f)return;
    const qty=Math.floor(Number(it.qty)||0);if(qty<1)return;
    if(it.variantId){
      const a=f.p.variants||[],idx=a.findIndex(v=>String(v.id)===String(it.variantId));
      if(idx>=0){a[idx].stock=Math.max(0,Number(a[idx].stock)||0)+qty;s.getRange(f.row,12).setValue(JSON.stringify(a));}
    }else{
      const current=Math.max(0,Number(s.getRange(f.row,5).getValue())||0);
      s.getRange(f.row,5).setValue(current+qty);
    }
    s.getRange(f.row,7).setValue(now_());
  });
}

/*
 * New authenticated POS endpoint.
 * Idempotency is stored in OrderIdempotency. Retrying the same local bill
 * returns the original result without touching stock again.
 */
function createPosBill_(p,session){
  ...
  try{
    ...
    stockReserved=reserveStockAtomic_(Array.isArray(p.items)?p.items:[]);
    const subtotal=Math.round(stockReserved.reduce((n,x)=>n+(Number(x.price)||0)*x.qty,0)*100)/100;
    const gstAmount=Math.round(subtotal*gstRate/100*100)/100;
    const total=Math.round((subtotal+gstAmount)*100)/100;
    const payment=normalizePayment_(p.payment,total);

    const os=ensurePosOrderSchema_();
    const id='PMT-POS-'+new Date().getFullYear()+'-'+Utilities.getUuid().slice(0,8).toUpperCase();
    const itemsJson=stockReserved.map(x=>({id:x.id,name:x.name,qty:x.qty,price:x.price,variantId:x.variantId||''}));
    os.appendRow([id,now_(),name,phone,JSON.stringify(itemsJson),total,payment.mode,'Confirmed','',subtotal,gstRate,gstAmount,pmtJson_(payment),key,'POS']);
    clearCache_();
    auditSafe_('pos_bill_create',id+' | '+key);
    const result={ok:true,id,orderId:id,total,subtotal,gstRate,gstAmount,payment,items:itemsJson};
    ...
    /* No partial order should survive. Stock writes are under the same lock;
       restore if a later validation step fails. */
...
function ensureIdempotencySheet_(){
  const db=DB();let s=db.getSheetByName('OrderIdempotency');
  if(!s){s=db.insertSheet('OrderIdempotency');s.getRange(1,1,1,3).setValues([['idempotencyKey','createdAt','resultJson']]);}
  return s;
}
function findIdempotency_(s,key){
  const last=s.getLastRow();if(last<2)return null;
  const vals=s.getRange(2,1,last-1,3).getValues();
  for(const r of vals)if(String(r[0])===key){try{return JSON.parse(String(r[2]||'{}'));}catch(e){return {ok:false,error:'Stored idempotency result is invalid',code:'IDEMPOTENCY_CORRUPT'};}}
  return null;
}
function saveIdempotency_(s,key,result){s.appendRow([key,now_(),JSON.stringify(result)]);}

/* ---------- MSG91 notifications ---------- */
function msg91Fetch_(url,options){
  const opts=Object.assign({muteHttpExceptions:true},options||{});
  const res=UrlFetchApp.fetch(url,opts);
  const code=res.getResponseCode();
  const text=res.getContentText();
  if(code<200||code>=300)throw Error('MSG91 '+code+': '+text.slice(0,500));
  return text;
}
function sendMsg91Sms_(phone,orderId,total){
  if(!phone)return;
  const auth=P('PMT_MSG91_AUTHKEY'),flow=P('PMT_MSG91_SMS_FLOW_ID');
  if(!auth||!flow)throw Error('MSG91 SMS properties are not configured');
  const body={template_id:flow,recipients:[{mobiles:'91'+phone,ORDER_ID:String(orderId),AMOUNT:String(total)}]};
  return msg91Fetch_('https://control.msg91.com/api/v5/flow',{
    method:'post',contentType:'application/json',headers:{authkey:auth},payload:JSON.stringify(body)
  });
}
function sendMsg91WhatsApp_(phone,orderId,total){
  if(!phone)return;
  const auth=P('PMT_MSG91_AUTHKEY'),integrated=P('PMT_MSG91_WA_INTEGRATED_NUMBER'),template=P('PMT_MSG91_WA_TEMPLATE_NAME');
  if(!auth||!integrated||!template)throw Error('MSG91 WhatsApp properties are not configured');
  const lang=P('PMT_MSG91_WA_TEMPLATE_LANG')||'en';
  const body={integrated_number:integrated,content_type:'template',payload:{to:'91'+phone,type:'template',template:{name:template,language:{code:lang,policy:'deterministic'},components:[{type:'body',parameters:[{type:'text',text:String(orderId)},{type:'text',text:String(total)}]}]}}};
  return msg91Fetch_('https://control.msg91.com/api/v5/whatsapp/whatsapp-outbound-message/',{
    method:'post',contentType:'application/json',headers:{authkey:auth},payload:JSON.stringify(body)
  });
}
function sendOrderConfirmationNotifications_(customerName,customerPhone,orderId,total){
  const text='Piyare Mobile Telecom: Order '+orderId+' confirmed. Total ₹'+Number(total||0).toLocaleString('en-IN')+'. Thank you, '+customerName+'.';
  if(customerPhone){
    try{sendMsg91Sms_(customerPhone,orderId,total);}catch(e){auditSafe_('sms_error',String(e));}
    try{sendMsg91WhatsApp_(customerPhone,orderId,total);}catch(e){auditSafe_('customer_whatsapp_error',String(e));}
  }
  const owner=P('PMT_OWNER_PHONE');
  if(owner){
    try{sendMsg91WhatsApp_(owner,orderId,total);}catch(e){auditSafe_('owner_whatsapp_error',String(e));}
  }
  const ns=S('Notifications');
  if(ns)ns.appendRow([now_(),'order_confirmed',text,customerPhone||'',orderId]);
}

/*
 * ROUTER CHANGES — add to existing doPost AFTER adminLogin and BEFORE the final
 * Unknown action response:
 *
 *   if(a==='createPosBill')return requireRole_(session,'Support')?createPosBill_(b.payload||{},session):forbidden_();
 *
 * The existing public createOrder_ should also use reserveStockAtomic_ inside its
 * existing LockService critical section if you want website checkout and POS to
 * share exactly the same atomic stock path. The POS endpoint above is already safe
 * for concurrent POS devices and retry-safe offline sync.
 */
