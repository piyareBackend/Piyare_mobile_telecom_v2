...
export default {
  async fetch(request, env) {
    ...
    const PMT_API = String(env.PMT_API_URL || DEFAULT_PMT_API);
    ...
    if (url.pathname === '/health') {
      try {
        const u = new URL(PMT_API);
        u.searchParams.set('action', 'publicProducts');
        u.searchParams.set('_health', PMT_VERSION);
        u.searchParams.set('_fresh', Date.now().toString());
        const backend = await fetch(u.toString(), {
          method: 'GET',
          redirect: 'follow',
          cache: 'no-store'
        });
        ...
        try {
          ...
          parsed = { ok: false, error: 'Backend returned non-JSON', status: backend.status };
        }
        ...
        const payload = {
          ...
          backend: backendOk
            ? { ok: true, service: 'pmt-backend', probe: 'publicProducts', productCount: Array.isArray(parsed.data) ? parsed.data.length : 0 }
            : { ok: false, error: parsed?.error || 'Backend health probe failed', code: parsed?.code || 'BACKEND_HEALTH_FAILED' }
        };
    ...
    if (url.pathname === '/api' || url.pathname.startsWith('/api/')) {
      ...
      try {
        const response = await fetch(target.toString(), cacheablePublic
    ...
    if (url.pathname === '/img') {
      ...
      if (!source) {
        return new Response(JSON.stringify({ ok: false, error: 'Missing image source', code: 'IMAGE_SOURCE_MISSING' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store', 'X-PMT-Version': PMT_VERSION }
        });
      }
      let src;
      try {
        src = new URL(canonicalDriveSource(source));
      } catch (_) {
        return new Response(JSON.stringify({ ok: false, error: 'Invalid image source', code: 'IMAGE_SOURCE_INVALID' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store', 'X-PMT-Version': PMT_VERSION }
        });
      }
      if (!['drive.google.com', 'drive.usercontent.google.com', 'lh3.googleusercontent.com'].includes(src.hostname)) {
        return new Response(JSON.stringify({ ok: false, error: 'Image source not allowed', code: 'IMAGE_SOURCE_NOT_ALLOWED' }), {
          status: 403,
          headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store', 'X-PMT-Version': PMT_VERSION }
        });
      }
      ...
      try {
        const response = await fetch(src.toString(), {
          cf: { image: { width, height, fit: 'scale-down', format: 'auto', quality: 82 } },
          redirect: 'follow'
        });
        if (!response.ok) {
          return new Response(JSON.stringify({ ok: false, error: 'Drive image unavailable', code: 'IMAGE_FETCH_FAILED', status: response.status }), {
        ...
        return new Response(JSON.stringify({ ok: false, error: String(err && err.message || err), code: 'IMAGE_PROXY_FAILED' }), {
          status: 502,
          headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store', 'X-PMT-Version': PMT_VERSION }
        });
      }
    ...
    return new Response('PMT Worker is running, but the ASSETS binding is unavailable in this preview/deployment.', {
      status: 503,
      headers: { 'Content-Type': 'text/plain', 'Cache-Control': 'no-store', 'X-PMT-Version': PMT_VERSION }
    });
  }
};
