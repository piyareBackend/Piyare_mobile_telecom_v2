...
function productImages_(v){let a=[];try{a=Array.isArray(v)?v:JSON.parse(String(v||"[]"));}catch(e){a=[];}return a.filter(x=>typeof x==='string'&&x.trim()).slice(0,4).map(x=>clean_(x,1200));}
...
function publicProducts_(){const a=moduleData_("Products",x=>({id:String(x[0]),name:String(x[1]),sku:String(x[2]),price:Number(x[3]||0),stock:Number(x[4]||0),minimum:Number(x[5]||0),updated:String(x[6]||""),icon:String(x[7]||"📱"),category:String(x[8]||"Accessories"),description:clean_(x[9]||"",5000),images:productImages_(x[10])})).filter(x=>x.stock>0&&x.category!=="__REMOVED__");return J({ok:true,items:a});}
...
function ownerModule_(name){
  ...
  if(name==="products")return J({ok:true,items:moduleData_("Products",x=>({id:String(x[0]),name:String(x[1]),sku:String(x[2]),price:Number(x[3]||0),stock:Number(x[4]||0),minimum:Number(x[5]||0),updated:String(x[6]||""),icon:String(x[7]||"📱"),category:String(x[8]||"Accessories"),description:clean_(x[9]||"",5000),images:productImages_(x[10])}))});
  ...
  if(name==="lowStock")return J({ok:true,items:moduleData_("Products",x=>({id:String(x[0]),name:String(x[1]),stock:Number(x[4]||0),minimum:Number(x[5]||0)})).filter(x=>x.stock<=x.minimum&&x.category!=="__REMOVED__")});
...
function createProduct_(p,session){const s=S("Products");if(!s)return J({ok:false,message:"Products sheet missing"});const name=clean_(p.name,100),sku=clean_(p.sku,80),price=Number(p.price),stock=Math.max(0,Number(p.stock)||0),minimum=Math.max(0,Number(p.minimum)||0),description=clean_(p.description||"",5000),images=productImages_(p.images);if(!name||!isFinite(price)||price<0)return J({ok:false,message:"Invalid product data"});const id="PRD-"+Utilities.getUuid().slice(0,8).toUpperCase();s.appendRow([id,name,sku,price,stock,minimum,now_(),clean_(p.icon||"📱",8),clean_(p.category||"Accessories",60),description,JSON.stringify(images)]);audit_("product_create",id);return J({ok:true,id});}
function updateProduct_(p,session){const s=S("Products");if(!s)return J({ok:false,message:"Products sheet missing"});const id=clean_(p.id,120),r=s.getDataRange().getValues();for(let i=1;i<r.length;i++)if(String(r[i][0])===id){if(p.name!=null)s.getRange(i+1,2).setValue(clean_(p.name,100));if(p.sku!=null)s.getRange(i+1,3).setValue(clean_(p.sku,80));if(p.price!=null)s.getRange(i+1,4).setValue(Math.max(0,Number(p.price)||0));if(p.stock!=null)s.getRange(i+1,5).setValue(Math.max(0,Number(p.stock)||0));if(p.minimum!=null)s.getRange(i+1,6).setValue(Math.max(0,Number(p.minimum)||0));if(p.icon!=null)s.getRange(i+1,8).setValue(clean_(p.icon,8));if(p.category!=null)s.getRange(i+1,9).setValue(clean_(p.category,60));if(p.description!=null)s.getRange(i+1,10).setValue(clean_(p.description,5000));if(p.images!=null)s.getRange(i+1,11).setValue(JSON.stringify(productImages_(p.images)));s.getRange(i+1,7).setValue(now_());audit_("product_update",id);return J({ok:true});}return J({ok:false,message:"Product not found"});}
function deleteProduct_(p,session){const s=S("Products");if(!s)return J({ok:false,message:"Products sheet missing"});const id=clean_(p.id,120),r=s.getDataRange().getValues();for(let i=1;i<r.length;i++)if(String(r[i][0])===id){s.deleteRow(i+1);audit_("product_delete",id);return J({ok:true});}return J({ok:false,message:"Product not found"});}
...
function lowStockCheck_(){try{const low=moduleData_("Products",x=>({name:String(x[1]),stock:Number(x[4]||0),minimum:Number(x[5]||0),category:String(x[8]||"")})).filter(x=>x.stock<=x.minimum&&x.category!=="__REMOVED__");if(low.length)notification_("low_stock",low.map(x=>x.name+" ("+x.stock+")").join(", "));}catch(e){auditSafe_("low_stock_error",String(e));}}
